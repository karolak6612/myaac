<script>
  ...
  onMount(async () => {
    try {
      const response = await fetch('/guilds?api=1');
      if (response.ok) {
  ...
  <ul class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
    {#each guilds as guild}
      <li class="border rounded p-4 shadow bg-white hover:bg-gray-50 transition">
        <a href="/guilds/{guild.name}" class="flex flex-col items-center text-center">
          <img src="{base}/images/guilds/{guild.logo}" alt="Logo" class="w-16 h-16 object-cover mb-2" />
